# https://github.com/ocadaruma/zsh-gcloud-prompt/blob/master/gcloud.zsh
...
function _set_zsh_gcloud_prompt() {
    ...
    ZSH_GCLOUD_PROMPT_COLOR="cyan"
    if [[ "{$project}" =~ "production" ]]; then
        ZSH_GCLOUD_PROMPT_COLOR="red"
    elif [[ "$project" =~ "staging" ]]; then
        ZSH_GCLOUD_PROMPT_COLOR="yellow"
    fi
